...
import { Loader2, Sparkles, FileCode2, Download, Copy, Check } from 'lucide-react'
...
import { useAI, type MultiFileGenerateResponse, type FileOutput } from '@/services/aiService'
...
const AppBuilder = () => {
  ...
  const [framework, setFramework] = useState<'react' | 'nextjs' | 'vue' | 'express' | 'fastapi'>('react')
  const [styling, setStyling] = useState<'tailwind' | 'css' | 'styled-components'>('tailwind')
  const [features, setFeatures] = useState<string[]>([])
  const [featureInput, setFeatureInput] = useState('')
  const [loading, setLoading] = useState(false)
  const [progress, setProgress] = useState(0)
  const [generatedApp, setGeneratedApp] = useState<MultiFileGenerateResponse | null>(null)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Failed to generate app',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
        ...
        <main className="container mx-auto px-4 py-8">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full mb-4">
              ...
              <span className="text-sm font-medium text-primary">Powered by DeepSeek Coder V2</span>
            </div>
            <h1 className="text-4xl font-bold tracking-tight mb-3">
              Build Apps with AI
            </h1>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              Describe your app and get production-ready code with multiple files, just like Cursor and Lovable
            </p>
          </div>
                ...
                {loading && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      ...
                      {progress >= 60 && progress < 90 && 'Creating components and files...'}
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {generatedApp ? `${generatedApp.files.length} files ready` : 'Your files will appear here'}
                    </CardDescription>
                  </div>
                  {generatedApp && (
                ...
                {loading && (
                  <div className="h-full flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-muted-foreground mt-2">This may take 3-5 minutes</p>
                    </div>
                  </div>
                )}
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <pre className="bg-muted p-3 rounded text-xs font-mono overflow-x-auto">
                              {JSON.stringify(generatedApp.dependencies, null, 2)}
                            </pre>
                          </div>
                          <Separator />
                          <div>
