...
import { Database, Check, Loader2, AlertCircle, Copy, Eye, EyeOff } from 'lucide-react'
...
import type { GetDatabaseResponse, ProvisionDatabaseResponse } from '@/types/database'
...
export function DatabasePanel({ appId, requiresDatabase = false, onDatabaseProvisioned }: DatabasePanelProps) {
  const { toast } = useToast()
  const [loading, setLoading] = useState(false)
  const [checking, setChecking] = useState(false)
  const [databaseInfo, setDatabaseInfo] = useState<GetDatabaseResponse | null>(null)
  const [provisionResult, setProvisionResult] = useState<ProvisionDatabaseResponse | null>(null)
  ...
  const provisionDatabase = async () => {
    if (!appId) {
      toast({
        title: 'Error',
        description: 'Please save your app first before provisioning a database',
        variant: 'destructive',
      })
      return
    }
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Failed to provision database',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
        ...
        {!databaseInfo?.has_database ? (
          <>
            <Alert>
              ...
              <AlertDescription>
                Get a dedicated PostgreSQL database with 1GB storage, automated backups, and instant provisioning
              </AlertDescription>
            </Alert>
          ...
          <div className="space-y-4">
            ...
            {provisionResult && (
              <div className="space-y-3">
                ...
                {provisionResult.schemas && provisionResult.schemas.length > 0 && (
                  <div>
                    <Label className="text-sm font-medium">
                      Initialized Tables
                    </Label>
                    <div className="mt-2 space-y-1">
                      {provisionResult.schemas.map((schema, idx) => {
                        const match = schema.match(/CREATE TABLE.*?(\w+)\s*\(/i)
                        const tableName = match ? match[1] : `Table ${idx + 1}`
                        return (
                ...
                <Alert>
                  <AlertDescription className="text-xs">
                    ...
                    <ul className="list-disc list-inside mt-2 space-y-1">
                      <li>Use this connection string in your app's environment variables</li>
