...
export default function SecurityScanner() {
  ...
  return (
    <div ref={containerRef} className="py-16 bg-gradient-to-br from-white via-red-50/30 to-orange-50/20">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-red-100/60 rounded-full border border-red-200 mb-4">
            ...
            <span className="text-red-700 font-medium text-sm">Security Analysis</span>
          </div>
          ...
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            Run a comprehensive security audit of your VPN connection. Our scanner checks multiple security vectors in real-time.
          </p>
        </div>
              ...
              <div className="text-center">
                <div className="relative w-32 h-32 mx-auto mb-4">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <div className={`text-2xl font-bold ${overallSecurity.color}`}>
                ...
                <button
                  ...
                  className={`w-full px-6 py-3 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 ${
                    isScanning
                      ...
                      : 'bg-gradient-to-r from-red-500 to-orange-600 text-white shadow-lg hover:shadow-xl hover:scale-105'
                  }`}
                  data-magnetic
                >
                  {isScanning ? (
              ...
              <div className="space-y-4">
                {[
                  { 
                    ...
                    value: securityChecks.filter(c => ['secure', 'warning', 'vulnerable'].includes(c.status)).length,
                    total: securityChecks.length,
                    color: 'text-blue-600'
                  },
                  { 
                    label: 'Secure Items', 
                    value: securityChecks.filter(c => c.status === 'secure').length,
                    total: securityChecks.length,
                    color: 'text-green-600'
                  },
                  { 
                    label: 'Warnings', 
                    value: securityChecks.filter(c => c.status === 'warning').length,
                    total: securityChecks.length,
                    color: 'text-yellow-600'
                  },
                  { 
                    label: 'Vulnerabilities', 
                    value: securityChecks.filter(c => c.status === 'vulnerable').length,
                    total: securityChecks.length,
                    color: 'text-red-600'
                  }
                ].map((stat, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-white/40 rounded-lg border border-red-200">
            ...
            {securityChecks.map((check) => {
              ...
              return (
                ...
                >
                  ...
                  {check.status !== 'pending' && check.status !== 'scanning' && (
