...
export default function InteractiveNetworkMap() {
  ...
  return (
    <div
      ref={containerRef}
      className='py-16 bg-linear-to-br from-white via-emerald-50/30 to-green-50/20'
    >
      <div className='container mx-auto px-4'>
        <div className='text-center mb-12'>
          ...
          <h2 className='text-4xl font-bold text-gray-800 mb-4'>
            Interactive{' '}
            <span className='bg-linear-to-br from-blue-600 to-emerald-600 bg-clip-text text-transparent'>
              Network Map
            </span>
          </h2>
          <p className='text-gray-600 max-w-2xl mx-auto text-lg'>
            Explore our global server network. Click on any server to see
            real-time performance metrics and connection details.
          </p>
        </div>
        ...
        <div className='max-w-6xl mx-auto'>
          <div className='bg-white/60 backdrop-blur-sm rounded-3xl border border-emerald-200 p-8 shadow-2xl'>
            ...
            <div
              ref={mapRef}
              className='relative w-full h-96 bg-linear-to-br from-blue-50 to-emerald-50 rounded-2xl border border-blue-200 overflow-hidden'
            >
              {/* Decorative grid */}
              ...
              <svg className='absolute inset-0 w-full h-full pointer-events-none'>
                {servers
                  ...
                  .map((server, index) =>
                    servers
                      .filter(
                        (s) => s.status === 'online' && s.id !== server.id,
                      )
                      .slice(0, 2)
                      .map((targetServer, targetIndex) => (
                        <line
              ...
              {servers.map((server) => (
                ...
                >
                  ...
                  {server.status === 'online' && (
                    <div
                      className={`server-pulse absolute inset-0 w-6 h-6 ${getStatusColor(server.status)} rounded-full`}
                    ></div>
                  )}

                  {/* Main server dot */}
                  <div
                    className={`server-dot relative w-4 h-4 ${getStatusColor(server.status)} rounded-full border-2 group-hover:scale-150 transition-all duration-300 shadow-lg`}
                  >
                    {/* Inner glow */}
                    ...
                    <div className='bg-white/95 backdrop-blur-sm rounded-lg border border-emerald-200 px-3 py-2 shadow-xl min-w-32'>
                      <div className='text-center'>
                        ...
                        <div className='flex items-center justify-center gap-1 mt-1'>
                          <div
                            className={`w-2 h-2 ${getStatusColor(server.status)} rounded-full`}
                          ></div>
            ...
            {selectedServer && (
              <div className='mt-6 bg-linear-to-br from-emerald-50 to-green-50 rounded-2xl border border-emerald-200 p-6'>
                <div className='flex items-start justify-between mb-4'>
                  <div>
                    ...
                    <div className='flex items-center gap-2 mt-2'>
                      <div
                        className={`w-3 h-3 ${getStatusColor(selectedServer.status)} rounded-full`}
                      ></div>
                  ...
                  <div className='bg-white/60 rounded-lg border border-emerald-200 p-4'>
                    ...
                    <div className='text-2xl font-bold text-gray-800'>
                      {selectedServer.status === 'online'
                        ? 'Low'
                        : selectedServer.status === 'high-load'
                          ? 'High'
                          : 'N/A'}
                    </div>
                  </div>
                ...
                {selectedServer.status === 'online' && (
                  <div className='mt-4 flex justify-center'>
                    <button className='px-6 py-2 bg-linear-to-br from-emerald-500 to-green-600 text-white rounded-lg hover:shadow-lg transition-all duration-300 font-medium'>
            ...
            <div className='mt-6 grid grid-cols-1 md:grid-cols-4 gap-4'>
              {[
                ...
                return (
                  <div
                    key={index}
                    className='bg-white/40 backdrop-blur-sm rounded-xl border border-emerald-200 p-4 text-center'
                  >
