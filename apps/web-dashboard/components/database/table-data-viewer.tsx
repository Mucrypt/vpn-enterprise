...
export function TableDataViewer({ tableName, schemaName, activeTenant, onClose }: TableDataViewerProps) {
  ...
  useEffect(() => {
    loadTableStructure();
    loadTableData();
  }, [tableName, schemaName, activeTenant, currentPage, sortColumn, sortDirection, searchQuery]);
  ...
  const loadTableData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/tenants/${activeTenant}/tables/${schemaName}.${tableName}/data?${params}`);
      if (response.ok) {
        const result = await response.json();
        setData(result.data || []);
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col items-center opacity-0 group-hover:opacity-100">
                        <span className={`text-xs ${sortColumn === column.name && sortDirection === 'asc' ? 'text-emerald-400' : 'text-gray-500'}`}>▲</span>
                        <span className={`text-xs ${sortColumn === column.name && sortDirection === 'desc' ? 'text-emerald-400' : 'text-gray-500'}`}>▼</span>
                      </div>
                    </div>
          ...
          {data.length === 0 && !loading && (
            <div className="text-center py-12">
              <div className="text-gray-400">
                {searchQuery ? 'No rows match your search' : 'No data in this table'}
              </div>
            </div>
          )}
        </div>
