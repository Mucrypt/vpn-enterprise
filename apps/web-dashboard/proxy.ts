  ...
  if (pathname.startsWith('/dashboard')) {
    ...
    if (!token && refreshCookie) {
      try {
        ...
        const resp = await fetch(
          `${request.nextUrl.origin}/api/v1/auth/refresh`,
          {
            method: 'POST',
            headers: {
    ...
    if (pathname.startsWith('/dashboard/admin')) {
      ...
      if (userRole !== 'super_admin' && userRole !== 'admin') {
        console.warn('[proxy] Non-admin tried to access admin route:', {
          userRole,
        })
    ...
    if (pathname.startsWith('/databases/admin')) {
      const userRole = request.cookies.get('user_role')?.value
      if (userRole !== 'super_admin' && userRole !== 'admin') {
        console.warn('[proxy] Non-admin tried to access database admin:', {
          userRole,
        })
