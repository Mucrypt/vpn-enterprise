...
const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://vpn-enterprise-api.vercel.app';
...
export default function VPNConfigPage() {
  ...
  const loadConfigs = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/vpn/configs?userId=${user.id}`);
  ...
  const loadUsage = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/vpn/usage?userId=${user.id}`);
      const data = await response.json();
      setUsage(data.usage);
    } catch (error) {
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-emerald-900">
                  {usage.total_mb} MB {usage.unlimited ? '' : `/ ${usage.limit_mb} MB`}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-emerald-200">
                <div>
                  ...
                  <p className="text-sm font-medium text-emerald-900">{formatBytes(usage.bytes_received)}</p>
                </div>
                <div>
                  <p className="text-xs text-emerald-700">Uploaded</p>
                  <p className="text-sm font-medium text-emerald-900">{formatBytes(usage.bytes_sent)}</p>
                </div>
              </div>
            </div>
          </CardContent>
  ...
  <Card className="border-emerald-300 bg-emerald-50">
          <CardHeader>
            ...
            <CardDescription>Generate a WireGuard configuration for your device</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <Label htmlFor="deviceName" className="text-blue-900">Device Name</Label>
              <Input
                id="deviceName"
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        <h3 className="font-semibold text-gray-900">{config.device_name}</h3>
                        {config.is_active ? (
                      ...
                      <div className="mt-2 space-y-1">
                        <p className="text-sm text-gray-600">
                          IP Address: <span className="font-mono text-gray-900">{config.allocated_ip}</span>
                        </p>
                        <p className="text-sm text-gray-600">
                        ...
                        {config.bytes_sent + config.bytes_received > 0 && (
                          <p className="text-sm text-gray-600">
                            Total Data: {formatBytes(config.bytes_sent + config.bytes_received)}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600">Download from Play Store</p>
              </div>
              <ExternalLink className="h-5 w-5 text-gray-400" />
            </a>
            ...
            >
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600">Download from official site</p>
              </div>
              <ExternalLink className="h-5 w-5 text-gray-400" />
            </a>
          </div>
