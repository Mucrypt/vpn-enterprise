...
router.post('/workspaces', async (req: AuthRequest, res: Response) => {
  ...
  try {
    const { app_id, name, memoryLimit, cpuLimit, diskLimit, timeoutMinutes } = req.body
    ...
    if (app_id) {
      ...
      if (app.user_id !== userId) {
        return res.status(403).json({ error: 'Access denied: You do not own this app' })
      }
      ...
      if (app.tenant_id) {
        console.log(`[TerminalAPI] App has database, fetching connection info...`)
        try {
    ...
    await pool.query(
      ...
      [workspaceId, userId, app_id || null, container.containerId, workspaceName]
    )
 ...
 */
router.get('/workspaces/:workspaceId', async (req: AuthRequest, res: Response) => {
  const pool = getDbPool()
  try {
 ...
 */
router.delete('/workspaces/:workspaceId', async (req: AuthRequest, res: Response) => {
  const pool = getDbPool()
  try {
    const { workspaceId } = req.params
    const userId = req.user?.id
 ...
 */
router.post('/workspaces/:workspaceId/exec', async (req: AuthRequest, res: Response) => {
  try {
    const { workspaceId } = req.params
 ...
 */
router.get('/preview/:workspaceId/info', async (req: AuthRequest, res: Response) => {
  try {
    const { workspaceId } = req.params
    const userId = req.user?.id
